...
const MyOrder = () => {
  ...
  useEffect(() => {
    if (loadedMyOrder && user?.email) {
      const filterCard = loadedMyOrder.filter(
    ...
    <div className="container mx-auto">
      ...
      <div className="">
        <h1 className="text-2xl md:text-4xl text-center my-10">
          {user?.displayName}s Ordered Items
        </h1>
        <div className="grid grid-cols-1 gap-5 pl-4 md:pl-40 lg:pl-0 lg:grid-cols-3">
          {updateUser.length ? (
            ...
            <>
              <div></div>
              <div className="lg:text-center lg:pl-0">
                Must be{" "}
                <span className="text-bold text-green-600 text-xl">
                  Hungry??
                </span>
                ..Why don't you order a food first......
              </div>
            </>
          )}
        </div>
