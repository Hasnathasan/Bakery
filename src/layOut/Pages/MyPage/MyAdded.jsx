...
const MyAdded = () => {
  ...
  useEffect(() => {
    if (loadedMyitem && user?.email) {
      const filterCard = loadedMyitem.filter(
    ...
    <div className="container mx-auto">
      ...
      <div>
        <h1 className="text-2xl md:text-4xl text-center my-10">
          {user?.displayName}s Added Items
        </h1>
        <div className="grid grid-cols-1  gap-5 pl-4 md:pl-40 lg:pl-0 lg:grid-cols-3">
          {updateItem.length ? (
                  ...
                  <CardBody className="text-center">
                    ...
                    >
                      ...
                      <div className="mt-1">
                        ...
                        / <span>{user?.quantity} pcs</span>
                      </div>
                    </Typography>
                    <Typography
                      color="blue-gray"
                      className="pb-5 font-medium font-serif"
                      textGradient
                    >
                      <div className="text-blue-gray-800 ">
                        <span className=" text-xl ">{user?.customerName}</span>{" "}
                        / {user?.email}{" "}
                      </div>
                      <div className="font-serif text-black text-xl">
                        Origin of food:{" "}
                        <span className="text-green-700 ">{user?.origin}</span>
                      </div>
                    </Typography>
            ...
            <>
              <div></div>
              <div className="lg:text-center pl-14 lg:pl-0">
                Please add a food item of your choice!!
              </div>
            </>
          )}
        </div>
